# Unified Preregistration v4.0 for Tau-Superblock Cosmology Research Program

## Based on Examination of Project Manuscripts (.tex Files)

**Preregistration Version**: 4.0 (Final)

**Date**: 2026-07-15

**Principal Investigator & Author**: William Brown, aka William Wylde, Grody Turdmongerson, @t98817
...
This v4.0 preregistration is based directly on the content and structure of the examined .tex files.
...
**Equation (1) — Generalized 5D EKK Line Element** 

$$d\tilde{s}^2 = g_{\mu\nu} dx^\mu dx^\nu + \phi^2 (d\tau + A_\mu dx^\mu)^2$$

**Complete Key**:

* $d\tilde{s}^2$ : 5D line element.
* $g_{\mu\nu}$ : 4D pseudo-Minkowski baseline.
* $\phi$ : Temporal substrate stiffness scalar.
* $\tau$ : Compactified timeline coordinate (Tau Cylinder).
* $A_\mu$ : Cross-domain flux vector potential.
* $d\tau$ : Differential along the compactified timeline.

**Equation (2) — Redshift as Wave-Function Dispersion** 

$$\lambda(z) = \lambda_0 \exp\left( \kappa_{\text{leak}} \int \frac{\rho_{\text{aeon}} R_\tau}{\langle\gamma\rangle_6 I} dt \right)$$

**Complete Key**:

* $\lambda(z), \lambda_0$ : Observed and emitted wavelengths.
* $\kappa_{\text{leak}}$ : Phase-leakage factor.
* $\rho_{\text{aeon}}$ : CMB aeon fabric density.
* $R_\tau$ : Radius of the 5D KK Tau Cylinder.
* $\langle\gamma\rangle_6$ : Averaged Lorentz factor over the six domains.
* $I$ : Normalization invariant.
* $dt$ : Proper time differential.

**Equation (3) — Hexametric Averaged Lorentz Factor** 

$$\langle\gamma\rangle_6 = \frac{1}{6} \sum_{i=1}^6 \frac{1}{\sqrt{1 - \beta_i^2}}$$

**Complete Key**: Domain-averaged Lorentz factor over the six domains.

**Equation (4) — Phase Recursion Relation** 

$$a_n = a_{n-1} + \left(-\frac{1}{2}\right)^{n-1} \prod_{j=1}^{n-1} a_j$$

**Complete Key**: State coefficients driving bounces between Absolute Past and Future nodes.

**Equation (5) — Photon–Higgs Commutation** 

$$[\hat{J}_\gamma , \hat{M}_{\text{Higgs}}] = i \hbar \omega_{\text{wind,eff}} I_{8\times8}$$

**Complete Key**: Photon angular momentum commutes with Higgs fabric mass operator under effective winding rate.

**Equation (6) — Effective Winding Rate** 

$$\omega_{\text{wind,eff}} = \omega_{\text{wind}} \left(1 - \epsilon \delta g \cdot f_{\text{feedback}} + \kappa_{\text{leak}} \frac{\rho_{\text{aeon}} R}{I}\right)$$

**Complete Key**: Modulates frame-dragging and phase-slip in the photon bounce mechanism.

**Additional Framework Elements (William Brown's Superblock Parameters)**:

* $n_{\text{hier}} = \log_7 \left(\frac{\tau}{\lambda_{313.1}}\right) \approx 45.8$ (hierarchical binding).
* $313.1\text{ MeV}$ geometric friction floor (minimal violation cost of the Exclusion Principle).

All equations are preserved under the conformal $n_{\text{hier}}$ mapping in the .tex files.

---

## 3. Hypotheses and Specific Predictions

**H1 (Photon Kinematics & Atemporality)**: Photons are maximal-excitation temporal switches executing discrete high-frequency bounces between $\Phi_8^P$ and $\Phi_8^F$ inside PK ergospheres. Discrete path-length quanta deposit into the Weyl tensor (gravity emergence). Macroscopic $d\tau_{\text{local}} = 0$ is the averaged observation of the refresh cycle.

**H2 (Redshift as Geometric Dispersion)**: Redshift follows the exponential form of Eq. (2) from wave-function dispersion across the six domains on $S^3$ supersphere + phase leakage along the 5D KK Tau Cylinder. No abstract $\Lambda$ required. Prediction: Non-Doppler residuals and 7-fold modulations in DESI/high-z data.

**H3 (8-Phase Engine & $G_2$ Clockwork)**: The 8-phase octonionic binary engine ($\text{Aut}(\mathbb{O}) \cong G_2$) operates from Superblock theory. Photon spin reversal at Phase 8 matches the mode structure. Prediction: 7-fold and 142857-cycle signatures.

**H4 (Tau Cylinder Emergent Time)**: Local time arrow emerges from directed phase advancement on successive slices of the 5D KK Tau Cylinder, enforced by the Topological Exclusion Principle. Global history is the static cycle-closed cylinder.

**H5 (Topological Exclusion Principle)**: Fermionic antisymmetry and the 313.1 MeV floor emerge topologically from forbidden reset-phase overlaps. Bosons close the 142857 cycle coherently.
...
## 4. Methodology and Investigative Methods
...
* Base all derivations on content in the examined project .tex files.

**Empirical Pipeline**:

* CMS OpenData: 7-fold resonance scans in dimuon/Higgs/photon channels with geometric floor isolation.
* DESI: Fit Eq. (2) form and search for 7-fold modulations.
* Casimir: Phase-dependent 1/7 harmonic analysis linked to 313.1 MeV floor.
* Statistical standards: Pre-specified cuts, backgrounds, and falsification thresholds as detailed in the project manuscripts.

All code, modules, and analytical outputs in the project repository form the primary record.

---

## 5. Specific Preregistered Tests

1. CMS 7-Fold Resonance Scan.
2. Photon Kinematics Signatures in CMS (discrete bounce deposition).
3. DESI Eq. (2) Dispersion Fit and 7-Fold Search.
4. Casimir Phase-Dependent Modulation linked to 313.1 MeV floor.
5. Hierarchical Self-Similarity across scales ($n_{\text{hier}}$).
6. Higgs Sector Signatures of the Exclusion Principle.
7. Strict pre-specified falsification thresholds from the project manuscripts.

---

## 6. Expected Outcomes, Timeline & Governance
...
## 7. References (Primary: Project Manuscripts)
